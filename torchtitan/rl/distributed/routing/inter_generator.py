...
class InterGeneratorRouter(Actor, Configurable):
    ...
    async def _pull_model_state_dict(
        ...
        async def _pull_one(h: _GeneratorHandle) -> dict[str, float]:
        ...
        profiles = await asyncio.gather(*[_pull_one(h) for h in self._generators])
        profiles = [profile for profile in profiles if profile]
        if not profiles:
            return {
                "nixl_prepare_seconds": 0.0,
                "nixl_completion_wait_seconds": 0.0,
            }
        return {
            key: max(profile[key] for profile in profiles)
