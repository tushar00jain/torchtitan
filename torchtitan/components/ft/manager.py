...
from torchtitan.components.ft.diloco import FaultTolerantModelArgs
...
def maybe_semi_sync_training(
    ...
    if ft_config.enable and semi_sync_method is not None:
        from torchft import local_sgd
        from torchtitan.components.ft.diloco import FaultTolerantTrainSpec
        ...
        if semi_sync_method.lower() == "diloco":
            ...
            model_args = cast(FaultTolerantModelArgs, model_args)
            if train_spec.fragment_fn:
                model_parts = train_spec.fragment_fn(
                    model, ft_config, model_args.n_layers
                )
            else:
                model_parts = [model]
